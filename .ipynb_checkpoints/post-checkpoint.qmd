...
The answer turns out to be: yes, there are several features that seem to be exactly preserved between layers, and it's quite a good sign that separately trained SAE's find the same subspace here.

:::{.columns}
::: {.column width="50%"}
![Decoder weight scatterplot](plots/decoder_weight_7_8_scatter.png){width=100%}
:::

::: {.column width="50%"}
![Right image](plots/decoder_weight_7_8_scatter_zoom.png){width=100%}
:::
:::



Overall, I like this method a lot more finding persistent features within a model, but there's no obvious way to translate it for different residual streams. This is where the idea of the bilinear product comes in. If two separate models have the exact same tokenizer, up to rearranging the indices of their token vocabularies. We can perhaps project the decoder vectors of their SAE's into the same space using their respective unembedding matrices, and then apply FAISS to get a list of related features. 
